...
adminApp.controller("dashboard-ctrl", function($http, $scope) {
	...
	var today = new Date();
	var monthDetailReport = String(today.getMonth() + 1).padStart(2, '0');
	var yearDetailReport = today.getFullYear();
	$scope.getTurnoverDetailReport(yearDetailReport, monthDetailReport);

	$scope.top5Orders = [];
	$http({
		url: "/api/report/top5OrdersByCreateDate",
		method: "GET"
	}).then(resp => {
		$scope.top5Orders = resp.data;
	})
	
	$scope.top5OrdersDetail = [];
	$http({
		url: "/api/report/top5OrderDetail",
		method: "GET"
	}).then(resp => {
		$scope.top5OrdersDetail = resp.data;
	})
	
	$scope.top6NewUsers = [];
	$http({
		url: "/api/report/top6NewUser",
		method: "GET"
	}).then(resp => {
		$scope.top6NewUsers = resp.data;		
	})
	
	$scope.topProduct = [];
	$http({
		url: "/api/report/topProduct",
		method: "GET"
	}).then(resp => {
